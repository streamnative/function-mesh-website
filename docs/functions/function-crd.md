...
| `retainOrdering` | The function consumes and processes messages in order. When you set `retainOrdering` to `effectively_once`, the runtime will set the subscription type to `FAILOVER`. By default, the subscription type is set to `SHARED`. |
