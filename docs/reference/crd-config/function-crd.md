...
Runtime logs include all logs generated by your pods. These logs provide information about the output for your functions. Function Mesh provides the following fields for the runtime logs in the CRD definition.
...
| `logConfig`| A key-name format option that is used to reference to a custom log configuration file. |
| `javaLog4JConfigFileType` | The Log4j configuration file type. Available options are `yaml`, `xml` and `init`. This option is only available for the Java runtime. |
