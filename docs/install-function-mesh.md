...
This document describes how to install Function Mesh based on your application in Kubernetes and how to start Mesh Worker service in case you want to use the [`pulsar-admin`](https://pulsar.apache.org/docs/en/pulsar-admin/) CLI tool to manage Pulsar Functions and connectors.
...
Function Mesh supports Mesh Worker service, which can forward requests to the Kubernetes cluster. After Mesh Worker service is started, users can use the [`pulsar-admin`](https://pulsar.apache.org/docs/en/pulsar-admin/) CLI tool to manage Pulsar Functions and connectors.

> **Note**
>
> Mesh Worker service is only available for Pulsar 2.8.0 or higher.

To start Mesh Worker service, follow these steps.

1. Add the following Mesh Worker service configuration to your `functions_worker.yml` configuration file.

    ```bash
    functionsWorkerServiceNarPackage: /YOUR-NAR-PATH/mesh-worker-service-{version}.nar
    ```
...
4. Verify whether the Mesh Worker service is started successfully.
